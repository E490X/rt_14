...
import { GET_BOTIM_LOGS } from "config/ApiNameConstant";
...
const BotimLogs = () => {
  ...
  const { totalCount, data, fetchData } = useFetchData(GET_BOTIM_LOGS, params, currentPageNumber);
  const {
    childCheckedState,
    parentChecked,
  ...
  function fetchDataofChats(params) {
    ApiUtils.getBotimChatByNumber(params)
      .then((res) => {
  ...
  const handleExportExcel = async (fromDate, toDate) => {
    try {
      ...
      const response = await ApiUtils.getMessagesByExcel(11, userDeviceIdAsNumber, fromDate, toDate, idsToSend);
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "botim-messages.xlsx");
  ...
  return (
    <>
      <MainCard content={false} title="Botim Messages Logs">
        <CardContent>
