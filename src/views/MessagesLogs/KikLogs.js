...
import { GET_KIK_LOGS } from "config/ApiNameConstant";
...
const KikLogs = () => {
  ...
  const {
    ...
    const handleExportExcel = async (fromDate, toDate) => {
        try {
          ...
          link.href = url;
          link.setAttribute("download", "kik-messages.xlsx");
  ...
  function fetchDataofChats(params) {
    ApiUtils.getKikByContactPersonName(params)
      .then((res) => {
  ...
  return (
    <>
      <MainCard content={false} title="KiK Messages Logs">
        <CardContent>
