...
import { GET_SIGNAL_LOGS } from "config/ApiNameConstant";
...
const SignalLogs = () => {
  ...
  function fetchDataofChats(params) {
    ApiUtils.getSignalByContactPersonName(params)
      .then((res) => {
  ...
  const handleExportExcel = async (fromDate, toDate) => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", "signal-messages.xlsx");
  ...
  const handleSearch = (fromDateISOString, toDateISOString) => {
    let filterData = { fromDate: fromDateISOString, toDate: toDateISOString };
    setFilterModel({ fromDate: fromDateISOString, toDate: toDateISOString });
    let paramUrl = getParamUrl(filterData, userDeviceIdAsNumber, currentPageNumber);
    fetchData(paramUrl);
  };
            ...
            <section className="msger">
              <main className="msger-chat">
                {chatData.length > 0 &&
                  chatData.map((chat) => (
                    <>
                      <div className={chat.messageType === "Incoming" ? "msg left-msg" : "msg right-msg"}>
