...
import { GET_SKYPE_LOGS } from "config/ApiNameConstant";
...
const SkypeLogs = () => {
  ...
  const handleExportExcel = async (fromDate, toDate) => {
      try {
        ...
        link.href = url;
        link.setAttribute("download", "skype-messages.xlsx");
  ...
  function fetchDataofChats(params) {
    ApiUtils.getSkypeByContactPersonName(params)
      .then((res) => {
  ...
  return (
    <>
      <MainCard content={false} title="Skype Messages Logs">
        <CardContent>
