...
import { GET_VIBER_LOGS } from "config/ApiNameConstant";
...
const ViberLogs = () => {
  ...
  const handleExportExcel = async (fromDate, toDate) => {
          try {
            ...
            link.href = url;
            link.setAttribute("download", "viber-messages.xlsx");
  ...
  function fetchDataofChats(params) {
    ApiUtils.getViberByContactPersonName(params)
      .then((res) => {
  ...
  return (
    <>
      <MainCard content={false} title="Viber Messages Logs">
        <CardContent>
