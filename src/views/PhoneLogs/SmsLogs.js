...
import { GET_SMS_LOGS } from "config/ApiNameConstant";
...
const SmsLogs = () => {
  ...
  function fetchDataofChats(params) {
    ApiUtils.getSmsLogs(params)
      .then((res) => {
        const dataTable = res.data.data.listResponse.map((data, index) => ({
          ...data,
          id: index + 1,
          time: extractTime(data.logDateTime),
          date: extractDate(data.logDateTime),
        }));
  ...
  const showChatModal = (params) => {
    setChatData([]);
    let paramsChatLogs;
    const hasPlus = params.row.number.includes("+");
    if (hasPlus) {
      const spiltNum = params.row.number.split("+")[1];
      paramsChatLogs = `DeviceUserId=${userDeviceIdAsNumber}&PhoneNumber=%2B${spiltNum}`;
    } else {
      paramsChatLogs = `DeviceUserId=${userDeviceIdAsNumber}&PhoneNumber=${params.row.number}`;
    }
    fetchDataofChats(paramsChatLogs);
    setOpen(true);
  };
  const columns = [
  ...
  const handleExportFile = (fromDateISOString, toDateISOString) => {
    ...
    ApiUtils.getSmsLogDetailsByExcel(paramsForExcel)
      .then((response) => {
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "smslog-details.xlsx");
  ...
  return (
    <>
      <MainCard content={false} title="SMS Logs">
        <CardContent>
